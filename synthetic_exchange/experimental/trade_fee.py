...
from synthetic_exchange.experimental.backtest_exchange import BacktestExchange as Exchange
from synthetic_exchange.experimental.dtype import OrderType, PositionAction, PriceType, Side as TradeType
...
class TokenAmount:
    ...
    def to_json(self) -> Dict[str, Any]:
        return {"token": self.token, "amount": str(self.amount)}
...
class TradeFeeBase(ABC):
    ...
    def new_perpetual_fee(
        ...
        fee_cls: Type[TradeFeeBase] = (
            AddedToCostTradeFee
            if position_action == PositionAction.PositionAction_Open or fee_schema.percent_fee_token is not None
            else DeductedFromReturnsTradeFee
        )
        return fee_cls(percent=percent, percent_token=percent_token, flat_fees=flat_fees or [])
    ...
    def get_fee_impact_on_order_cost(
        self, order_candidate: "OrderCandidate", exchange: "Exchange"  # noqa: F821
    ...
    def fee_amount_in_token(
        ...
        if self.percent != S_DECIMAL_0:
            ...
            if self._are_tokens_interchangeable(quote, token):
                ...
                conversion_pair: str = combine_trading_pair(base=quote, quote=token)
        ...
        for flat_fee in self.flat_fees:
            if self._are_tokens_interchangeable(flat_fee.token, token):
                ...
                conversion_pair: str = combine_trading_pair(base=flat_fee.token, quote=token)
                conversion_rate: Decimal = self._get_exchange_rate(conversion_pair, exchange, rate_source)
        ...
        if self.percent != S_DECIMAL_0:
            ...
            if order_candidate.order_collateral is None or fee_token != order_candidate.order_collateral.token:
                ...
                if fee_token == token:
                    ...
                    exchange_pair = combine_trading_pair(token, fee_token)  # buy order token w/ pf token
