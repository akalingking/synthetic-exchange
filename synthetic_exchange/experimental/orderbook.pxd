...
from synthetic_exchange.experimental.orderbook_entry cimport OrderBookEntry
...
cdef class OrderBook(PubSub):
	...
	cdef c_apply_numpy_diffs(self, np.ndarray[np.float64_t, ndim=2] bids_array, np.ndarray[np.float64_t, ndim=2] asks_array)
	cdef c_apply_numpy_snapshot(self, np.ndarray[np.float64_t, ndim=2] bids_array, np.ndarray[np.float64_t, ndim=2] asks_array)
