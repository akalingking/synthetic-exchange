...
from synthetic_exchange.util import safe_ensure_future


class OrderBookTracker:
    def __init__(self, trading_pairs: list):
    ...
    async def _init_orderbooks(self):
        for index, trading_pair in enumerate(self._trading_pairs):
            self._orderbooks[trading_pair] = await self._init_orderbook(trading_pair)
