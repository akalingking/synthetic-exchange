...
from synthetic_exchange.experimental.event import EventOrderFill
...
cdef class EventLogger(EventListener):
    def __init__(self, event_source: Optional[str] = None):
        ...
        self._order_fill_logged_events = deque()
        self._logged_events = {EventOrderFill: self._order_fill_logged_events}
        self._waiting = {}
        self._wait_returns = {}

    @property
    def event_log(self) -> List[any]:
        return list(self._generic_logged_events) + list(self._order_fill_logged_events)
